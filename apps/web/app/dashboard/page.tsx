  ...
  return <PageShell flush><main className="ops-room">
    ...
    {stats?.alltimeSummary&&(
      <section className="kpi-strip kpi-alltime">
        ...
        <Ledger className="ops-ledger" size="sm" label="◈ DAYS ACTIVE" value={stats.alltimeSummary.dateRange.earliest?Math.max(1,Math.ceil((Date.now()-new Date(stats.alltimeSummary.dateRange.earliest).getTime())/86400000)):0}/>
    ...
    <section className="primary-grid">
      <div className="request-plane"><Heading code="01" title="REQUEST VELOCITY / 48H" link="/logs"/>
        {chart.length?<div className="main-chart"><ResponsiveContainer width="100%" height="100%"><AreaChart data={chart}><defs><linearGradient id="flow" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#45b881" stopOpacity=".24"/><stop offset="1" stopColor="#45b881" stopOpacity="0"/></linearGradient><linearGradient id="tok" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#4dabf7" stopOpacity=".18"/><stop offset="1" stopColor="#4dabf7" stopOpacity="0"/></linearGradient></defs><CartesianGrid stroke="#252525" vertical={false}/><XAxis dataKey="time" tickFormatter={clock} stroke="#555" fontSize={9}/><YAxis yAxisId="req" stroke="#555" fontSize={9}/><YAxis yAxisId="tok" orientation="right" stroke="#4dabf7" fontSize={9} tickFormatter={compact}/><Tooltip contentStyle={{background:"#090b0b",border:"1px solid #444",borderRadius:0,fontSize:11}} labelFormatter={v=>new Date(String(v)).toLocaleString()}/><Area yAxisId="req" type="stepAfter" dataKey="requests" stroke="#45b881" fill="url(#flow)" strokeWidth={1.5}/><Area yAxisId="req" type="stepAfter" dataKey="errors" stroke="#e5484d" fill="transparent" strokeWidth={1}/><Area yAxisId="tok" type="stepAfter" dataKey="totalTokens" stroke="#4dabf7" fill="url(#tok)" strokeWidth={1}/></AreaChart></ResponsiveContainer></div>:<div className="empty-cell">NO REQUEST TELEMETRY</div>}
      </div>
