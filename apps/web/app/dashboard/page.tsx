...
interface Stats { requestsToday:number; requestsLast5h:number; totalRequests:number; totalTokens:number; averageLatencyMs:number; errorRate:number; estimatedCost:number; activeProviders:number; requestsOverTime:Hour[]; requestsByApiKeyOverTime:KeyHour[]; usageByProvider:Usage[]; usageByModel:Usage[]; usageByApiKey:Usage[] }
...
const clock=(v:string)=>new Date(v).toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"});

function health(provider:ProviderRow, logs:LogRow[]):Health {
  if(!provider.enabled)return "incident";
  const recent=logs.filter(x=>x.provider===provider.name&&Date.now()-new Date(x.createdAt).getTime()<900000).slice(0,8);
...
export default function DashboardPage(){
  const [stats,setStats]=useState<Stats|null>(null),[providers,setProviders]=useState<ProviderRow[]>([]),[logs,setLogs]=useState<LogRow[]>([]),[quota,setQuota]=useState<Quota|null>(null);
  const [loading,setLoading]=useState(true),[error,setError]=useState(""),[updated,setUpdated]=useState<Date|null>(null);
  const load=useCallback(async()=>{setLoading(true);try{const [s,p,l,q]=await Promise.all([apiFetch<Stats>("/admin/stats"),apiFetch<ApiEnvelope<ProviderRow[]>>("/admin/providers"),apiFetch<ApiEnvelope<LogRow[]>>("/admin/logs"),apiFetch<Quota>("/admin/quota")]);setStats(s);setProviders(p.data);setLogs(l.data);setQuota(q);setError("");setUpdated(new Date())}catch(e){setError(e instanceof Error?e.message:"Control plane unavailable")}finally{setLoading(false)}},[]);
  ...
  const gemini429=logs.filter(isGemini429); const resetAt=quota?.exactProviderResetAt??quota?.estimatedFiveHourResetAt; const quotaActive=Boolean(quota?.active||(resetAt&&new Date(resetAt)>new Date()));
  const incidents=[...(quotaActive?[`Z.AI LIMIT ACTIVE — RESET ${resetAt?new Date(resetAt).toLocaleString():"PENDING"}`]:[]),...(gemini429.length?[`GEMINI 429 DEGRADATION — ${gemini429.length} RECORDED`]:[]),...providerRows.filter(x=>x.h==="incident").map(x=>`${x.p.name.toUpperCase()} UNAVAILABLE`)];
  ...
  return <PageShell flush><main className="ops-room">
    ...
    <section className="primary-grid">
      <div className="request-plane"><Heading code="01" title="REQUEST VELOCITY / 48H" link="/logs"/>
        {chart.length?<div className="main-chart"><ResponsiveContainer width="100%" height="100%"><AreaChart data={chart}><defs><linearGradient id="flow" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#45b881" stopOpacity=".24"/><stop offset="1" stopColor="#45b881" stopOpacity="0"/></linearGradient></defs><CartesianGrid stroke="#252525" vertical={false}/><XAxis dataKey="time" tickFormatter={clock} stroke="#555" fontSize={9}/><YAxis stroke="#555" fontSize={9}/><Tooltip contentStyle={{background:"#090b0b",border:"1px solid #444",borderRadius:0,fontSize:11}} labelFormatter={v=>new Date(String(v)).toLocaleString()}/><Area type="stepAfter" dataKey="requests" stroke="#45b881" fill="url(#flow)" strokeWidth={1.5}/><Area type="stepAfter" dataKey="errors" stroke="#e5484d" fill="transparent" strokeWidth={1}/></AreaChart></ResponsiveContainer></div>:<div className="empty-cell">NO REQUEST TELEMETRY</div>}
      </div>
      <aside className="health-column">
        <div><Heading code="02A" title="QUOTA CLOCK"/><div className={`quota-terminal ${quotaActive?"bad":"good"}`}><b>{quotaActive?"BLOCKED":"AVAILABLE"}</b><span>Z.AI / CODING POOL</span><strong>{quotaActive&&resetAt?new Date(resetAt).toLocaleString():"NO ACTIVE RESET"}</strong><small>{quota?.exactProviderResetAt?"PROVIDER-EXACT TIMESTAMP":quotaActive?"FIVE-HOUR ESTIMATE":"LAST EVENT EXPIRED"}</small></div></div>
        <div><Heading code="02B" title="PROVIDER HEALTH"/>{providerRows.length?providerRows.map(({p,h,rows})=><div className="provider-line" key={p.id}><i style={{background:C[h]}}/><b>{p.name}</b><span>{h}</span><Semaphore segments={(rows.length?rows.map((r):SemaphoreSegment=>({status:r.status==="success"?"operational":isGemini429(r)?"degraded":"incident",label:clock(r.createdAt),detail:r.errorCode??undefined})):Array.from({length:12},()=>({status:"empty"})))} segmentWidth={4} segmentHeight={13} gap={2} radius={0} pulse={false}/></div>):<div className="empty-cell">NO PROVIDERS CONFIGURED</div>} {gemini429.length>0&&<div className="gemini-line">GEMINI 429 <b>DEGRADED</b><span>{clock(gemini429[0]!.createdAt)} LAST</span></div>}</div>
      </aside>
    </section>
