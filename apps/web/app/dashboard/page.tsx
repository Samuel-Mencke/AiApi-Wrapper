...
export default function DashboardPage(){
  ...
  return <PageShell flush><main className="ops-room">
    ...
    <section className="lower-grid">
      <div><Heading code="03" title="MODEL COST LEDGER"/>{modelRows.length?<div className="table-wrap"><table className="ops-table"><thead><tr><th>MODEL / PROVIDER</th><th>REQ</th><th>IN TOK</th><th>OUT TOK</th><th>$/M IN</th><th>$/M OUT</th><th>REALIZED</th><th>RETAIL VALUE</th></tr></thead><tbody>{modelRows.map(r=><tr key={r.id}><td><b>{r.label}</b><small>{r.provider}</small></td><td className="fx-data">{compact(r.requests)}</td><td className="fx-data">{compact(r.inputTokens)}</td><td className="fx-data">{compact(r.outputTokens)}</td><td className="fx-data">{money(r.inputPrice)}</td><td className="fx-data">{money(r.outputPrice)}</td><td className="fx-data">{money(r.estimatedCost)}</td><td className="fx-data">{money(r.retail)}</td></tr>)}</tbody></table></div>:<div className="empty-cell">NO MODEL COST ACTIVITY</div>}</div>
