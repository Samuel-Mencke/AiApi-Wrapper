...
export function AppSidebar({ collapsed, onToggleCollapsed }: { collapsed: boolean; onToggleCollapsed: () => void }) {
  ...
  return (
    <aside
      className={cn(
        "fixed inset-y-0 left-0 z-20 hidden border-r border-white/[0.06] bg-[#0f0f10] md:block",
        mounted && "transition-[width] duration-300 ease-out",
        collapsed ? "w-20" : "w-56 xl:w-64"
      )}
    >
      <div className={cn("flex h-16 items-center border-b border-white/[0.06] px-4", collapsed ? "justify-center" : "justify-between gap-3")}>
        ...
        <button
          className={cn(
            ...
            collapsed && "absolute -right-4 top-5 bg-[#111111] shadow-xl shadow-black/30"
          )}
              ...
              return (
                <Link
                  ...
                  className={cn(
                    ...
                    active && "bg-[#222226] text-zinc-100"
                  )}
                >
